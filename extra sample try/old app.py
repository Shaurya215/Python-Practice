...
# CSV must have columns: cuisine, diet, prep_time, ingredients, name, image_url
# Put cuisine_updated.csv in Recipe folder, or adjust path. recipe_final.csv has a different schema.
df = pd.read_csv(os.path.join(os.path.dirname(__file__), "cuisine_updated.csv"))
...
vectorizer.fit(df["cuisine"].astype(str) + " " + df["diet"].astype(str) + " " + df["prep_time"].astype(str) + " " + df["ingredients"].astype(str))
